...
def process_keyword_data_transpose(input_file, output_file=None):
    """
    关键词数据分析转置处理函数
    处理所有工作表，保持sheet数量一致
    ...
    try:
        ...
        if output_file is None:
            ...
            output_file = f"{base_name}_{current_date}_关键词转置完成.xlsx"
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("使用方法: python keyword_data_transpose.py <输入文件> [输出文件]")
        print("示例: python keyword_data_transpose.py 数据文件.xlsx")
        print("示例: python keyword_data_transpose.py 数据文件.xlsx 输出文件.xlsx")
        return
    ...
    results = process_keyword_data_transpose(input_file, output_file)
