...
"""
完整转置处理工具
同时处理"信源数据分析"和"关键词数据分析"两个工作表
将第一行品牌转置为一列，其他数据按照对应关系展示
"""
...
def transpose_source_data_sheet(ws):
    ...
    print(f"信源数据分析提取完成，总共 {len(data_rows)} 行数据")
...
def transpose_keyword_data_sheet(ws):
    ...
    print(f"关键词数据分析提取完成，总共 {len(data_rows)} 行数据")
    return pd.DataFrame(data_rows)
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("使用方法: python complete_transpose_both_sheets.py <输入文件> [输出文件]")
        print("示例: python complete_transpose_both_sheets.py 数据文件.xlsx")
        print("示例: python complete_transpose_both_sheets.py 数据文件.xlsx 输出文件.xlsx")
        return
