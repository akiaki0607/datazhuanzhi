...
def process_internal_report_transpose(input_file, output_file=None):
    """
    对内报表转置处理函数
    ...
    try:
        ...
        for col_idx in range(1, ws.max_column + 1):
            ...
            main_headers.append(cell.value)
        ...
        print(f"主标题行: {main_headers[:10]}...")  # 显示前10个
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("使用方法: python process_internal_report.py <输入文件> [输出文件]")
        print("示例: python process_internal_report.py 数据文件.xlsx")
        print("示例: python process_internal_report.py 数据文件.xlsx 输出文件.xlsx")
        return
    ...
    df = process_internal_report_transpose(input_file, output_file)
