...
def correct_transpose(input_file, output_file=None):
    """
    正确的转置处理函数
    ...
    try:
        ...
        if output_file is None:
            ...
            output_file = f"{base_name}_{current_date}_正确转置完成.xlsx"
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("使用方法: python correct_transpose.py <输入文件> [输出文件]")
        print("示例: python correct_transpose.py 数据文件.xlsx")
        print("示例: python correct_transpose.py 数据文件.xlsx 输出文件.xlsx")
        return
    ...
    df = correct_transpose(input_file, output_file)
